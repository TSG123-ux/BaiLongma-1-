...
export function initAIVideoMode() {
  ...
  const panel = el("aivideo-panel");
  if (!panel) return;
  const queueEl=el("aivideo-queue"), stage=el("aivideo-stage"), stageEmpty=el("aivideo-stage-empty"),
  ...
  let active=false, jobs=[], selId=null, images=[], submitting=false;

  const toastEl=document.createElement("div"); toastEl.className="aivideo-toast"; document.body.appendChild(toastEl);
  let toastTimer=null;
  ...
  function mediaUrl(u){ const s=String(u||""); if(!s) return ""; return s.charAt(0)==="/" ? (API+s) : s; }
  function modeLabel(m){ return m==="flf"?"首尾帧":(m==="image"?"图生视频":"文生视频"); }
  function jobById(id){ for(let i=0;i<jobs.length;i++){ if(jobs[i].id===id) return jobs[i]; } return null; }

  // —— 感知同步：把「面板开关 + 提示词草稿」实时回传后端，让 agent 能直接看到用户在框里写了什么 ——
  let draftTimer=null, lastDraftSent=null;
  function syncDraft(immediate){
    clearTimeout(draftTimer);
    const doSync=function(){
      const payload=JSON.stringify({ open:active, prompt:(promptInput.value||"") });
  ...
  function renderQueue(){
    queueEl.innerHTML="";
    if(!jobs.length){ const em=document.createElement("div"); em.className="aivideo-queue-empty"; em.textContent="还没有生成任务"; queueEl.appendChild(em); return; }
    jobs.forEach(function(j){
      ...
      if(j.status==="done"){
        const v=document.createElement("video"); v.className="thumb"; v.src=mediaUrl(j.videoUrl); v.muted=true; v.playsInline=true; v.preload="metadata"; fr.appendChild(v);
        const pl=document.createElement("div"); pl.className="play"; pl.textContent="▶"; fr.appendChild(pl);
        if(j.dur){ const d=document.createElement("div"); d.className="dur"; d.textContent=j.dur+"s"; fr.appendChild(d); }
      } else if(j.status==="gen"){
        const orb=document.createElement("div"); orb.className="av-orb"; orb.innerHTML="<i></i><i></i>"; fr.appendChild(orb);
        const gb=document.createElement("div"); gb.className="genbadge"; gb.textContent="生成中"; fr.appendChild(gb);
        const gt=document.createElement("div"); gt.className="gentime"; gt.dataset.start=String(j.start||Date.now()); gt.textContent="0:00"; fr.appendChild(gt);
      } else {
        const x=document.createElement("div"); x.className="x"; x.textContent="!"; fr.appendChild(x);
      }
      if(j.status!=="gen"){ const rm=document.createElement("button"); rm.className="rm"; rm.textContent="×"; rm.onclick=function(e){ e.stopPropagation(); removeJob(j.id); }; fr.appendChild(rm); }
      t.appendChild(fr);
  ...
  function tickTimers(){ const now=Date.now(); const list=queueEl.querySelectorAll(".gentime"); for(let i=0;i<list.length;i++){ let s=Math.floor((now-Number(list[i].dataset.start))/1000); if(s<0)s=0; list[i].textContent=Math.floor(s/60)+":"+String(s%60).padStart(2,"0"); } }
  ...
  function download(){
    if(!selId) return; dlBtn.disabled=true; const old=dlBtn.textContent; dlBtn.textContent="保存中…";
  ...
  function renderDropzone(){
    ...
    images.forEach(function(src,i){
      const cell=document.createElement("div"); cell.className="av-imgcell";
      const im=document.createElement("img"); im.src=src; cell.appendChild(im);
      const r=document.createElement("div"); r.className="role"; r.textContent=images.length===2?(i===0?"首帧":"尾帧"):"参考图"; cell.appendChild(r);
      const rm=document.createElement("button"); rm.className="rm"; rm.textContent="×"; rm.onclick=function(e){ e.stopPropagation(); images.splice(i,1); renderDropzone(); updateMode(); }; cell.appendChild(rm);
      dropzone.appendChild(cell);
    });
    if(images.length<2){ const add=document.createElement("div"); add.className="av-addcell"; add.innerHTML='<span class="plus">+</span><span>图片</span><small>点击/拖拽/粘贴</small>'; add.onclick=function(){ fileInput.click(); }; dropzone.appendChild(add); }
  }
  var hadImages=false;
  function updateMode(){
    const m=images.length>=2?"flf":(images.length===1?"image":"text");
  ...
  fileInput.addEventListener("change", function(e){ const f=e.target.files&&e.target.files[0]; if(f){ const rd=new FileReader(); rd.onload=function(){ addImage(String(rd.result||"")); }; rd.readAsDataURL(f); } e.target.value=""; });
  ...
  dropzone.addEventListener("drop", function(e){ const f=e.dataTransfer&&e.dataTransfer.files&&e.dataTransfer.files[0]; if(f&&f.type.indexOf("image/")===0){ const rd=new FileReader(); rd.onload=function(){ addImage(String(rd.result||"")); }; rd.readAsDataURL(f); } });
  document.addEventListener("paste", function(e){
    if(!active) return; const cd=e.clipboardData||window.clipboardData; const items=cd&&cd.items; if(!items) return;
    for(let i=0;i<items.length;i++){ if(items[i].type.indexOf("image")===0){ const blob=items[i].getAsFile(); var rd=new FileReader(); rd.onload=function(){ addImage(String(rd.result||"")); }; rd.readAsDataURL(blob); e.preventDefault(); break; } }
  });

  const PROMPT_MIN=46, PROMPT_MAX=160;
  function autoGrow(){
    if(!promptInput.clientWidth){ promptInput.style.height=""; return; } // 面板隐藏(宽0)时测量会拿到错误的 scrollHeight，跳过，交给 CSS min-height
    promptInput.style.height="auto";
    const b=promptInput.offsetHeight-promptInput.clientHeight;
  ...
  function submitGenerate(){
    ...
    fetch(API+"/aivideo/generate",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({ prompt:prompt, images:images.slice(0,2), ratio:ratioSel.value, resolution:resSel.value, duration:Number(durSel.value)||5 })})
      ...
      .then(function(res){
        ...
        if(!res.ok || !res.d || !res.d.ok){ const d=res.d||{}; composeErr.textContent=d.guide||d.error||"提交失败"; composeErr.hidden=false; return; }
  ...
  function handle(data){
    data=data||{}; const action=data.action||"show";
    ...
    const job=jobById(data.jobId); if(!job) return;
